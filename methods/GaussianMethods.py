...
def OneSingleGaussian(input, array_alpha, num_of_train_images=None, data_for_train=None, im_show=True):
    ...
    if num_of_train_images is None:
        ...
        # zero means all images used as test => cam be useful when you input another training data
        assert (num_of_train_images >= 0 and num_of_train_images <= len(input))

    data = input[:num_of_train_images]

    if data_for_train is None:
        matrix_mean = np.mean(data,0).astype(np.float64)
        matrix_std = np.std(data, 0).astype(np.float64)
    else:
        matrix_mean = np.mean(data_for_train, 0).astype(np.float64)
        matrix_std = np.std(data_for_train, 0).astype(np.float64)
    ...
    data_for_test = input[num_of_train_images:]

    rows, cols = input.shape[1:]
    gt_test = np.zeros((len(array_alpha), len(data_for_test), rows, cols), dtype=np.bool)
    for i, alpha in enumerate(array_alpha):
        sys.stdout.write("\r>  Computing for alpha = {:.2f} ... {}%".format(alpha, i*100/len(array_alpha)))
        sys.stdout.flush()
        gt_frames = np.where(np.abs(data_for_test - matrix_mean) >= (alpha * (matrix_std + 2)), 1, 0)
        gt_test[i, ...] = gt_frames
    print("\n ---------------------------------------------- \n")
