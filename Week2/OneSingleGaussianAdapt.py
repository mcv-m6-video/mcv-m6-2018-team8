...
def OneSingleGaussianAdapt(input, alpha_params, rho_params, im_show=True):
    ...
    for id_a, alpha in enumerate(alpha_params):
        for id_r, rho in enumerate(rho_params):
            ...
            sys.stdout.write("\r>  Computing for alpha = {:.2f} and rho = {:.2f} ... {}%".format(alpha, rho, (total_id+1)*100 / (len(alpha_params)*len(rho_params))))
            sys.stdout.flush()

            # for k in range(len(data2)):
            #     cond = np.abs(data2[k] - matrix_mean) >= (alpha * (matrix_std + 2))
            #     mean_val = (rho * data2[k]) + ((1-rho) * matrix_mean_array[k])
            #     std_val = np.sqrt(rho * np.float_power(data2[k] - matrix_mean, 2) + np.float_power((1 - rho)*matrix_std, 2))
            #
            #     gt_frames[k] = np.where(cond, 1, 0)
            #     matrix_mean_array[k] = np.where(cond, matrix_mean, mean_val)
            #     matrix_std_array[k] = np.where(cond, matrix_std, std_val)
