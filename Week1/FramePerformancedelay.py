# -*- coding: utf-8 -*-
"""
Created on Mon Feb 26 22:29:04 2018
...
def FramePerformancedelay(gt, res, array_of_delays):
    TP_listlist = []
    total_foreground_listlist = []
    fscore_listlist = []
    for j in range(0,len(array_of_delays)):
        

        TP_list = []
        total_foreground_list = []
        fscore_list = []
        framedelay = array_of_delays[j] 
        for i in range(len(res)):
            ...
            gt_th = np.roll(gt_th, framedelay,axis=0) #Here we desychronize
        ...
        TP_listlist.append(TP_list)
        total_foreground_listlist.append(total_foreground_list)
        fscore_listlist.append(fscore_list)
    return TP_listlist, total_foreground_listlist, fscore_listlist